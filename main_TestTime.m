...
%         T_mse = zeros(1,num_Frame);
        ...
        for i_Montecarlo=1:num_Montecarlo
            ...
            for i_Frame=1:length(Time(:,1));
                ...
                %对齐检测
                if method==1||method==3
                    Result_Alignment=DetectionOfDataAlignment(Index_Alignment,Measure,num_XCoord,num_YCoord,gate_Alignment,Area,Temp_XCoord,Temp_YCoord,gridWidth,num_Channel,i_Frame);
                ...
                %优选检测检测
                if method==2||method==3
                    Result_Elect=DetectionOfDataElect(Index_Min,Index_Max,Measure,num_XCoord,num_YCoord,gate_Alignment,Area,Temp_XCoord,Temp_YCoord,gridWidth,num_Channel,num_Grid);
...
%             end
            ...
            if flag_Track==1
                ...
                if method==1||method==3
                    [l,T_mse]=MyTrack(Area,Target_Location,Velocity,gridWidth,saveFolder_AlignmentResult);
                end
                if method==2||method==3
                    [l,T_mse]=MyTrack(Area,Target_Location,Velocity,gridWidth,saveFolder_ElectResult);
                end
                if l == 0
