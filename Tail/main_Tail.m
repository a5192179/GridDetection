...
%% Basic Parameters
Result_Detection = [2000,0;1900,0;1800,0;1700,0;];
Velocity=[-100,0];
y_num = 250; x_num = 250; 

T= 1; % the interval of scanning
total_time = length(Result_Detection(:,1))*T; 
...
Save_data_detection=zeros(250,250,50);%【修改】20150124-李帅-保存每次产生的data_detection
...
Pd = (1+alpha/(N_aver*(1+10^(SNR/10))))^(-N_aver); % detection probability 《Fundamental of Radar Sigal Processing》p.266 
...
for ii = 1:1%mont_num
    ii
     %% 
     [target_state,target_meas] = non_maneuvering(Result_Detection,Velocity,total_time); % the target location（丙）
